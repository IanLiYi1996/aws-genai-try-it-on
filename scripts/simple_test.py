...
if __name__ == "__main__":
    etl_endpoint_name = "knowledgebase-endpoint-0305"
    source_bucket_name = (
        "knowledge-base-model-output-195275664417-us-west-2"
    )
    source_key = "test_input/test_3pages.pdf"
    result_bucket_name = (
        "knowledge-base-model-output-195275664417-us-west-2"
    )
    portal_bucket_name = (
    ...
    # Process PDF (will automatically choose appropriate method)
    all_text = pdf_loader.process_pdf(source_key)

    # Save the extracted text to a local file
    output_filename = (
        f"{os.path.splitext(os.path.basename(source_key))[0]}_extracted.txt"
    )
